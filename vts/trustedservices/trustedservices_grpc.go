...
func (o *GRPC) SetEndorsementsActive(
	ctx context.Context,
	req *proto.SetEndorsementsActiveRequest,
) (*proto.SetEndorsementsActiveResponse, error) { // nolint:gocritic
	o.logger.Debugw("SetEndorsementsActive", "set-active", req.SetActive, "tenant-id", req.TenantId)

	var elmq lifecycle.Query
	if err := elmq.FromCBOR(req.Data); err != nil {
		return setEndorsementsActiveErrorResponse(err), nil
	}

	if err := elmq.Valid(); err != nil {
		return setEndorsementsActiveErrorResponse(fmt.Errorf("invalid ELM query: %w", err)), nil
	}

	switch {
	case elmq.EnvironmentSelector != nil:
		if err := o.setEndorsementsActiveUsingEnv(&elmq, req.SetActive); err != nil {
			return setEndorsementsActiveErrorResponse(err), nil
		}
	case elmq.RimSelector != nil:
		if err := o.setEndorsementsActiveUsingIDs(&elmq, req.SetActive); err != nil {
			return setEndorsementsActiveErrorResponse(err), nil
		}
	default:
	...
	return setEndorsementsActiveSuccessResponse(), nil
}

func (o *GRPC) setEndorsementsActiveUsingEnv(query *lifecycle.Query, setActive bool) error {
...
func (o *GRPC) setEndorsementsActiveUsingIDs(query *lifecycle.Query, setActive bool) error {
	if err := query.Valid(); err != nil {
		return err
	}
...
func setEndorsementsActiveSuccessResponse() *proto.SetEndorsementsActiveResponse {
	return &proto.SetEndorsementsActiveResponse{
		Status: &proto.Status{
			Result: true,
		},
	}
...
func setEndorsementsActiveErrorResponse(err error) *proto.SetEndorsementsActiveResponse {
	return &proto.SetEndorsementsActiveResponse{
		Status: &proto.Status{
			Result:      false,
			ErrorDetail: fmt.Sprintf("%v", err),
