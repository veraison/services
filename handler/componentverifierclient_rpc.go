...
func (s *ComponentVerifierClientHandlerRPCServer) AppraiseComponentEvidence(args AppraiseComponentEvidenceArgs, resp *[]byte) error {
	attestationResult, err := s.Impl.AppraiseComponentEvidence(args.Evidence, args.MediaType, args.Nonce)

	*resp, err = json.Marshal(attestationResult)
...
func (c *ComponentVerifierClientHandlerRPCClient) AppraiseComponentEvidence(evidence []byte, mediaType string, nonce []byte) (*ear.Appraisal, error) {
	var (
		resp   []byte
		err    error
		result ear.Appraisal
	)
	...
	if err != nil {
		return nil, fmt.Errorf("failed to unmarshal appraisal result: %w", err)
	}
