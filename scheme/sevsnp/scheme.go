...
func (o *Implementation) GetReferenceValueIDs(
	...
	vt, err := extractTripleFromClaims(claims)
	if err != nil {
		return nil, handler.BadEvidence(err)
	}

	evMeasurements, err := transformValueTripleToMeasurementsMap(vt)
	if err != nil {
		return nil, handler.BadEvidence(err)
	}
	...
	return []*comid.Environment{
		{
			Class: &comid.Class{
				ClassID: vt.Environment.Class.ClassID,
			},
			Instance: instance,
		},
	}, err
...
func extractTripleFromClaims(claims map[string]any) (*comid.ValueTriple, error) {
...
func transformClaimsToMeasurementsMap(claims map[string]any) (map[uint64]comid.Measurement, error) {
	...
	return transformValueTripleToMeasurementsMap(vt)
}
