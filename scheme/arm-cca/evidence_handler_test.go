...
func Test_AppraiseEvidence_Platform_ok(t *testing.T) { // nolint: dupl
	...
	endorsementsBytes, err := os.ReadFile("test/platform/endorsements.json")
	require.NoError(t, err)
...
func Test_AppraiseEvidence_mismatch_refval_meas(t *testing.T) { // nolint: dupl
	...
	var endorsemementsArray []string
	endorsementsBytes, err := os.ReadFile("test/platform/mismatch-refval-endorsements.json")
	require.NoError(t, err)
	err = json.Unmarshal(endorsementsBytes, &endorsemementsArray)
	require.NoError(t, err)
...
func Test_AppraiseEvidence_mismatch_refval_cfg(t *testing.T) { // nolint: dupl
	...
	var endorsemementsArray []string
	endorsementsBytes, err := os.ReadFile("test/platform/mismatch-cfg-endorsements.json")
	require.NoError(t, err)
	err = json.Unmarshal(endorsementsBytes, &endorsemementsArray)
	require.NoError(t, err)
...
func Test_ExtractVerifiedClaims_ok(t *testing.T) {
	...
	require.NoError(t, err)
	taEndValBytes, err := os.ReadFile("test/platform/ta-endorsements.json")
	require.NoError(t, err)
	scheme := &EvidenceHandler{}
...
func Test_ValidateEvidenceIntegrity_invalid_key(t *testing.T) {
	tokenBytes, err := os.ReadFile("test/evidence/cca-token.cbor")
	require.NoError(t, err)
	taEndValBytes, err := os.ReadFile("test/platform/invalid-key-ta-endorsements.json")
	require.NoError(t, err)
	scheme := &EvidenceHandler{}
	token := proto.AttestationToken{
		TenantId: "1",
