...
func (p *Provisioner) SetEndorsementsState(tenantID string, data []byte, setActive bool) error {
	aReq := &proto.SetEndorsementsStateRequest{TenantId: tenantID, Data: data, SetActive: setActive}
	aRes, err := p.VTSClient.SetEndorsementsState(context.Background(), aReq)
	if err != nil {
		return fmt.Errorf("change endorsements activation state (setting active = %v) failed: %w", setActive, err)
	}

	if !aRes.GetStatus().Result {
		return fmt.Errorf(
			"change endorsements activation state (setting active = %v) failed: %s",
