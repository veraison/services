...
DEPLOYMENT_CERTS_DIR=${VERAISON_ROOT}/certs
DEPLOYMENT_LOGS_DIR=${VERAISON_ROOT}/logs
DEPLOYMENT_STORES_DIR=${VERAISON_ROOT}/stores
DEPLOYMENT_CONFIG_DIR=${VERAISON_ROOT}/config
...
function check_requirements() {
	...
	if ! which bash > /dev/null; then
		echo -e "$_ERROR: which must be installed."
		exit 1
	fi
	...
	local _sort=$(which gsort 2>/dev/null)
	if [[ $_sort == "" ]]; then
		_sort="sort"
	fi

	if ! $_sort --help | grep -- -V &>/dev/null; then
		echo -e "$_ERROR: GNU sort must be installed (on MacOS install coreutils)"
		exit 1
	fi

	if [[ "$(which go 2>/dev/null)" == "" ]]; then
		echo -e "$_ERROR: Go toolchain (at least version 1.19) must be installed."
		exit 1
	fi

	if ! printf '%s\n' 1.19 "$(go version | grep -o -E '[0-9.]+' | head -n1)" | \
		$_sort -C -V; then

		echo -e "$_ERROR: Go version must be at least 1.19."
		exit 1
	fi
...
function init_sqlite_stores() {
	${DEPLOYMENT_BIN_DIR}/veraison init-sqlite-stores
}

function init_clients() {
	_init_client evcli github.com/veraison/evcli/v2@40dfa3d3
	_init_client cocli github.com/veraison/corim/cocli@fc4fa6f4
	_init_client pocli github.com/veraison/pocli@2843c65b
}
...
function help() {
	...
	[-e] [-f] init-certificates | init-certs [NAME_TEMPLATE ROOT_CERT ROOT_CERT_KEY]
	    Initialize x509 certificates for the deployment, ether by generating new
