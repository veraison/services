...
option for `follow` command (see above). There is also shorter alias `logs` for
this command. E.g.
...
Complete delete logs from CloudWatch. This is mostly useful for debugging.
...
Restarts CloudWatch agent on running service EC2 instances. This is necessary
